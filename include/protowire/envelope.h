...
struct AppError {
  ...
  AppError& WithField(std::string field,
                      std::string err_code,
                      std::string msg,
                      std::vector<std::string> args = {});
...
struct Envelope {
  ...
  static Envelope Err(int32_t status,
                      std::string code,
                      std::string message,
                      std::vector<std::string> args = {});
...
inline Envelope MakeErr(int32_t status,
                        std::string code,
                        std::string message,
                        std::vector<std::string> args = {}) {
  return Envelope::Err(status, std::move(code), std::move(message), std::move(args));
}
inline Envelope MakeTransportErr(std::string err) {
  return Envelope::TransportErr(std::move(err));
}

inline AppError NewAppError(std::string code,
                            std::string message,
                            std::vector<std::string> args = {}) {
