...
const google::protobuf::Descriptor* LoadConfigDescriptor(
...
int main(int argc, char** argv) {
  ...
  double u_mib_per_sec =
      (static_cast<double>(pxf_text.size()) * u_iters) /
      (1024.0 * 1024.0) / u_seconds;
  std::printf(
      "{\"port\":\"cpp\",\"op\":\"unmarshal\",\"ns_per_op\":%lld,"
      "\"mib_per_sec\":%g,\"iterations\":%lld,\"bytes\":%zu}\n",
      u_ns_per_op, u_mib_per_sec, u_iters, pxf_text.size());
