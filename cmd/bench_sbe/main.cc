...
const google::protobuf::FileDescriptor* LoadFile(const std::string& fds_bytes,
...
int main(int argc, char** argv) {
  ...
  auto fresh_order = [&]() {
    return std::unique_ptr<google::protobuf::Message>(factory.GetPrototype(order_desc)->New());
  };
  ...
  auto order = fresh_order();
  {
    ...
    set("order_id",
        [&](const Reflection* r, const auto* fd) { r->SetUInt64(order.get(), fd, 1001); });
    set("symbol",
        [&](const Reflection* r, const auto* fd) { r->SetString(order.get(), fd, "AAPL"); });
    set("price", [&](const Reflection* r, const auto* fd) { r->SetInt64(order.get(), fd, 19150); });
    set("quantity",
        [&](const Reflection* r, const auto* fd) { r->SetUInt32(order.get(), fd, 100); });
    set("side", [&](const Reflection* r, const auto* fd) {
      r->SetEnumValue(order.get(), fd, 1);  // SIDE_SELL
    });
    set("active", [&](const Reflection* r, const auto* fd) { r->SetBool(order.get(), fd, true); });
    set("weight",
        [&](const Reflection* r, const auto* fd) { r->SetDouble(order.get(), fd, 0.85); });
    set("score", [&](const Reflection* r, const auto* fd) { r->SetFloat(order.get(), fd, 2.5f); });
