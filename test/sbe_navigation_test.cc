...
TEST_F(SbeNavigation, ViewComposite) {
  ...
  pb::Message* price_msg =
      r->MutableMessage(trade.get(), desc_->FindFieldByName("price"));
  const pb::Descriptor* price_desc = price_msg->GetDescriptor();
  price_msg->GetReflection()->SetInt64(
      price_msg, price_desc->FindFieldByName("mantissa"), 12345);
  price_msg->GetReflection()->SetInt32(
      price_msg, price_desc->FindFieldByName("exponent"), -2);
...
TEST_F(SbeNavigation, ViewGroup) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    fill->GetReflection()->SetInt64(
        fill, fill_desc->FindFieldByName("fill_price"), 1000 + i);
    fill->GetReflection()->SetUInt32(
        fill, fill_desc->FindFieldByName("fill_qty"), 10u + i);
  }
...
TEST_F(SbeNavigation, ViewBytes) {
  ...
  t2->GetReflection()->SetString(
      t2.get(), desc_->FindFieldByName("signature"),
...
TEST_F(SbeNavigation, MarshalUnmarshalRoundTrip) {
  ...
  pb::Message* price =
      r->MutableMessage(orig.get(), desc_->FindFieldByName("price"));
  price->GetReflection()->SetInt64(
      price, price->GetDescriptor()->FindFieldByName("mantissa"), 999);
  price->GetReflection()->SetInt32(
      price, price->GetDescriptor()->FindFieldByName("exponent"), 1);

  r->SetUInt64(orig.get(), desc_->FindFieldByName("qty"), 7);
  r->SetString(orig.get(), desc_->FindFieldByName("signature"),
               std::string("\x01\x02\x03\x04", 4));

  const pb::FieldDescriptor* fills_fd = desc_->FindFieldByName("fills");
  pb::Message* fill = r->AddMessage(orig.get(), fills_fd);
  fill->GetReflection()->SetInt64(
      fill, fill->GetDescriptor()->FindFieldByName("fill_price"), 500);
  fill->GetReflection()->SetUInt32(
      fill, fill->GetDescriptor()->FindFieldByName("fill_qty"), 2);
  ...
  std::string sig;
  EXPECT_EQ(r->GetStringReference(
                *got, desc_->FindFieldByName("signature"), &sig),
            std::string("\x01\x02\x03\x04", 4));
  const pb::Message& gprice =
      r->GetMessage(*got, desc_->FindFieldByName("price"));
  EXPECT_EQ(gprice.GetReflection()->GetInt64(
                gprice, gprice.GetDescriptor()->FindFieldByName("mantissa")),
            999);
