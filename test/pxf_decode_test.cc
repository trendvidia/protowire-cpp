...
enum_field = STATUS_ACTIVE
  ...
  std::string scratch;
  EXPECT_EQ(r->GetStringReference(*msg,
                                  desc_->FindFieldByName("string_field"),
                                  &scratch),
            "hello");
...
nested_field {
  ...
  EXPECT_EQ(sub.GetReflection()->GetStringReference(
            ...
            "inner");
  EXPECT_EQ(sub.GetReflection()->GetInt32(
...
nullable_bool = true
  ...
  const pb::Reflection* r = msg->GetReflection();
  const pb::Message& ws = r->GetMessage(
      *msg, desc_->FindFieldByName("nullable_string"));
  std::string scratch;
