...
class FixedResolver : public protowire::pxf::TypeResolver {
 public:
  ...
  const pbuf::Descriptor* FindMessageByURL(std::string_view url) override {
    ...
    std::string_view name = pos == std::string_view::npos ? url
                                                          : url.substr(pos + 1);
...
payload {
  ...
  const pbuf::Message& any =
      r->GetMessage(*msg, desc_->FindFieldByName("payload"));
  std::string scratch;
  ...
  std::unique_ptr<pbuf::Message> inner(
      factory_->GetPrototype(detail_desc_)->New());
  ASSERT_TRUE(inner->ParseFromString(packed));
  EXPECT_EQ(inner->GetReflection()->GetInt32(
                *inner, detail_desc_->FindFieldByName("code")),
            42);
...
TEST_F(PxfAny, EncodeAnySugar) {
  ...
  std::unique_ptr<pbuf::Message> inner(
      factory_->GetPrototype(detail_desc_)->New());
  inner->GetReflection()->SetInt32(inner.get(),
                                   detail_desc_->FindFieldByName("code"), 7);
  inner->GetReflection()->SetString(
      inner.get(), detail_desc_->FindFieldByName("reason"), "boom");
  std::string packed;
  ASSERT_TRUE(inner->SerializeToString(&packed));

  pbuf::Message* any =
      r->MutableMessage(msg.get(), desc_->FindFieldByName("payload"));
  any->GetReflection()->SetString(
      any, any->GetDescriptor()->FindFieldByName("type_url"),
      "any.test.ErrorDetail");
  any->GetReflection()->SetString(
      any, any->GetDescriptor()->FindFieldByName("value"), packed);
