...
TEST_F(PxfAnnotations, TimestampDefaultApplied) {
  ...
  EXPECT_EQ(ts.GetReflection()->GetInt64(ts, ts_desc->FindFieldByName("seconds")),
            1705314600);
  EXPECT_EQ(ts.GetReflection()->GetInt32(ts, ts_desc->FindFieldByName("nanos")),
            0);
}
...
TEST_F(PxfAnnotations, DurationDefaultApplied) {
  ...
  // 1h30m = 5400 seconds.
  EXPECT_EQ(dur.GetReflection()->GetInt64(dur,
                                          dur_desc->FindFieldByName("seconds")),
            5400);
}
...
TEST_F(PxfAnnotations, WrapperDefaultsApplied) {
  ...
  EXPECT_EQ(label.GetReflection()->GetString(
                label, label.GetDescriptor()->FindFieldByName("value")),
            "default");
  ...
  EXPECT_EQ(shards.GetReflection()->GetInt32(
                shards, shards.GetDescriptor()->FindFieldByName("value")),
            3);
  ...
  EXPECT_EQ(active.GetReflection()->GetBool(
                active, active.GetDescriptor()->FindFieldByName("value")),
            true);
}
