...
std::optional<Timestamp> ParseRFC3339(std::string_view s) {
  ...
  if (!ParseUInt(s.substr(0, 4), year) ||
      !ParseUInt(s.substr(5, 2), month) ||
      !ParseUInt(s.substr(8, 2), day) ||
      !ParseUInt(s.substr(11, 2), hour) ||
      !ParseUInt(s.substr(14, 2), minute) ||
      !ParseUInt(s.substr(17, 2), second)) {
    return std::nullopt;
  }
  ...
  if (tz == 'Z' || tz == 'z') {
    ...
    int oh, om;
    if (!ParseUInt(s.substr(pos + 1, 2), oh) ||
        !ParseUInt(s.substr(pos + 4, 2), om)) {
      return std::nullopt;
    }
...
std::string FormatRFC3339Nano(int64_t seconds, int32_t nanos) {
  ...
  char buf[64];
  if (nanos == 0) {
    std::snprintf(buf, sizeof(buf), "%04lld-%02u-%02u" "T%02d:%02d:%02dZ",
  ...
  std::snprintf(buf, sizeof(buf), "%04lld-%02u-%02u" "T%02d:%02d:%02d.%0*dZ",
                static_cast<long long>(y), m, d, hh, mm, ss, width, n);
  return std::string(buf);
}
