...
AppError& AppError::WithField(std::string field, std::string err_code,
                              std::string msg,
                              std::vector<std::string> args) {
  details.push_back(FieldError{std::move(field), std::move(err_code),
                               std::move(msg), std::move(args)});
  return *this;
}
...
Envelope Envelope::Err(int32_t status, std::string code, std::string message,
                       std::vector<std::string> args) {
  Envelope e;
  e.status = status;
