...
Status SetScalar(Message* msg, const FieldDescriptor* fd, const ValuePtr& v, Position pos);
...
Status DecodeMessage(Message* msg,
                     const std::vector<EntryPtr>& entries,
                     std::string path_prefix,
                     DecodeContext* ctx);

Status DecodeMessageValueInto(
    Message* sub, const ValuePtr& v, Position pos, std::string path_prefix, DecodeContext* ctx) {
  ...
  if (!bv) {
    return PosError(pos, "expected '{' for message field of " + std::string(d->full_name()));
  }
  ...
  if (IsAny(d) && ctx->resolver != nullptr && !bv->entries.empty()) {
    if (auto* head = Get<EntryPtr, Assignment>(bv->entries.front())) {
      if (head->key == "@type") {
        ...
        const Descriptor* inner_desc = ctx->resolver->FindMessageByURL(type_url);
        if (!inner_desc) {
          return PosError(head->pos, "cannot resolve Any type \"" + type_url + "\"");
        }
        ...
        sub->GetReflection()->SetString(
            sub, sub->GetDescriptor()->FindFieldByName("type_url"), std::move(type_url));
        sub->GetReflection()->SetString(
            sub, sub->GetDescriptor()->FindFieldByName("value"), std::move(packed));
        return Status::OK();
      }
    }
  }
...
Status SetScalar(Message* msg, const FieldDescriptor* fd, const ValuePtr& v, Position pos) {
  ...
  switch (fd->cpp_type()) {
    ...
    case FieldDescriptor::CPPTYPE_BOOL: {
      ...
      return PosError(pos, "expected bool for field \"" + std::string(fd->name()) + "\"");
    }
    case FieldDescriptor::CPPTYPE_INT32: {
      auto* i = Get<ValuePtr, IntVal>(v);
      if (!i) return PosError(pos, "expected integer for " + std::string(fd->name()));
      int32_t n;
      if (!ParseInteger<int32_t>(i->raw, n)) return PosError(pos, "invalid int32: " + i->raw);
      r->SetInt32(msg, fd, n);
      return Status::OK();
    }
    case FieldDescriptor::CPPTYPE_INT64: {
      ...
      if (!i) return PosError(pos, "expected integer for " + std::string(fd->name()));
      int64_t n;
      if (!ParseInteger<int64_t>(i->raw, n)) return PosError(pos, "invalid int64: " + i->raw);
      r->SetInt64(msg, fd, n);
      return Status::OK();
    }
    case FieldDescriptor::CPPTYPE_UINT32: {
      ...
      if (!i) return PosError(pos, "expected integer for " + std::string(fd->name()));
      uint32_t n;
      if (!ParseInteger<uint32_t>(i->raw, n)) return PosError(pos, "invalid uint32: " + i->raw);
    ...
    case FieldDescriptor::CPPTYPE_UINT64: {
      ...
      if (!i) return PosError(pos, "expected integer for " + std::string(fd->name()));
      uint64_t n;
      if (!ParseInteger<uint64_t>(i->raw, n)) return PosError(pos, "invalid uint64: " + i->raw);
    ...
    case FieldDescriptor::CPPTYPE_DOUBLE: {
      double d;
      if (auto* fv = Get<ValuePtr, FloatVal>(v)) {
        if (!ParseDouble(fv->raw, d)) return PosError(pos, "invalid float: " + fv->raw);
      } else if (auto* iv = Get<ValuePtr, IntVal>(v)) {
        if (!ParseDouble(iv->raw, d)) return PosError(pos, "invalid float: " + iv->raw);
      } else {
    ...
    case FieldDescriptor::CPPTYPE_ENUM: {
      ...
      if (auto* id = Get<ValuePtr, IdentVal>(v)) {
        ...
        if (!ev) {
          return PosError(
              pos,
              "unknown enum value \"" + id->name + "\" for " + std::string(enum_d->full_name()));
        }
      ...
      return PosError(pos, "expected enum name or number for " + std::string(fd->name()));
    }
...
Status AddRepeatedScalar(Message* msg, const FieldDescriptor* fd, const ValuePtr& v, Position pos) {
  const Reflection* r = msg->GetReflection();
  switch (fd->cpp_type()) {
    case FieldDescriptor::CPPTYPE_STRING: {
      if (auto* s = Get<ValuePtr, StringVal>(v)) {
...
Status DecodeListInto(Message* msg,
...
Status DecodeMapInto(Message* msg,
                     const FieldDescriptor* fd,
                     const std::vector<EntryPtr>& entries,
                     std::string path_prefix,
                     DecodeContext* ctx) {
  ...
  for (const EntryPtr& e : entries) {
    ...
    if (!m) {
      return PosError(EntryPos(e),
                      "map field \"" + std::string(fd->name()) + "\" expects 'key: value' entries");
    }
    if (Get<ValuePtr, NullVal>(m->value)) {
      return PosError(m->pos,
                      "null is not allowed as map value in \"" + std::string(fd->name()) + "\"");
    }
    ...
    if (val_fd->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE) {
      ...
      Status st = DecodeMessageValueInto(sub, m->value, m->pos, path_prefix, ctx);
      if (!st.ok()) return st;
    } else {
...
Status DecodeAssignment(Message* msg,
  ...
  if (!fd) {
    return PosError(pos, "unknown field \"" + key + "\" in " + std::string(desc->full_name()));
  }
...
Status DecodeBlockEntry(Message* msg,
                        const Block& block,
                        std::string path_prefix,
                        DecodeContext* ctx) {
  const Descriptor* desc = msg->GetDescriptor();
  ...
  if (!fd) {
    return PosError(block.pos,
                    "unknown field \"" + block.name + "\" in " + std::string(desc->full_name()));
  }
  if (fd->cpp_type() != FieldDescriptor::CPPTYPE_MESSAGE) {
    return PosError(block.pos,
                    "field \"" + block.name + "\" is not a message — block syntax forbidden");
  }
  if (fd->is_map()) {
    return PosError(block.pos, "map field \"" + block.name + "\" must use 'name = { ... }' syntax");
  }
  if (fd->is_repeated()) {
    return PosError(block.pos, "repeated field \"" + block.name + "\" must use list syntax");
  }
  ...
  if (IsAny(sub->GetDescriptor()) && ctx->resolver != nullptr && !block.entries.empty()) {
...
Status DecodeMessage(Message* msg,
  ...
  for (const EntryPtr& e : entries) {
    if (auto* a = Get<EntryPtr, Assignment>(e)) {
      Status st = DecodeAssignment(msg, a->key, a->value, a->pos, path_prefix, ctx);
      if (!st.ok()) return st;
    } else if (auto* b = Get<EntryPtr, Block>(e)) {
      Status st = DecodeBlockEntry(msg, *b, path_prefix, ctx);
      if (!st.ok()) return st;
    } else {
      return PosError(EntryPos(e), "map entry ':' is only valid inside a map context");
    }
  }
  return Status::OK();
}
...
// in decode_fast.cc.
Status DecodeDocumentInto(const Document& doc,
                          Message* msg,
