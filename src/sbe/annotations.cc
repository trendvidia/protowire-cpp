...
std::optional<uint32_t> ScanFieldsUint32(const pb::Message& opts,
                                         int field_number) {
...
std::optional<std::string> ScanFieldsString(const pb::Message& opts,
                                            int field_number) {
  const pb::Reflection* r = opts.GetReflection();
  std::vector<const pb::FieldDescriptor*> fields;
  r->ListFields(opts, &fields);
  for (const pb::FieldDescriptor* fd : fields) {
    if (fd->number() == field_number &&
        fd->cpp_type() == pb::FieldDescriptor::CPPTYPE_STRING) {
      std::string scratch;
...
std::optional<uint32_t> ScanUnknownUint32(const pb::Message& opts,
                                          int field_number) {
...
std::optional<std::string> ScanUnknownString(const pb::Message& opts,
                                             int field_number) {
  const pb::UnknownFieldSet& uf = opts.GetReflection()->GetUnknownFields(opts);
  for (int i = 0; i < uf.field_count(); ++i) {
    const pb::UnknownField& u = uf.field(i);
...
std::optional<uint32_t> GetUint32Option(const pb::Message& opts,
                                        int field_number) {
  if (auto v = ScanFieldsUint32(opts, field_number); v.has_value()) return v;
  return ScanUnknownUint32(opts, field_number);
}

std::optional<std::string> GetStringOption(const pb::Message& opts,
                                           int field_number) {
  if (auto v = ScanFieldsString(opts, field_number); v.has_value()) return v;
  return ScanUnknownString(opts, field_number);
}

std::optional<uint32_t> GetFileUint32Option(const pb::FileDescriptor* fd,
                                            int field_number) {
  return GetUint32Option(fd->options(), field_number);
}
std::optional<uint32_t> GetMessageUint32Option(const pb::Descriptor* md,
                                               int field_number) {
  return GetUint32Option(md->options(), field_number);
}
std::optional<uint32_t> GetFieldUint32Option(const pb::FieldDescriptor* fd,
                                             int field_number) {
  return GetUint32Option(fd->options(), field_number);
}
std::optional<std::string> GetFieldStringOption(const pb::FieldDescriptor* fd,
                                                int field_number) {
