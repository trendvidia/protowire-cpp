...
void EnsureAndWriteScalar(std::vector<uint8_t>& buf, size_t offset,
                          const FieldTemplate& ft, uint64_t v_bits) {
  while (buf.size() < offset + ft.size) buf.push_back(0);
  uint8_t* p = buf.data() + offset;
  switch (ft.size) {
    case 1: p[0] = static_cast<uint8_t>(v_bits); break;
    case 2: WriteU16(p, static_cast<uint16_t>(v_bits)); break;
    case 4: WriteU32(p, static_cast<uint32_t>(v_bits)); break;
    case 8: WriteU64(p, v_bits); break;
  }
}
...
void WriteFixedString(std::vector<uint8_t>& buf, size_t offset,
                      const FieldTemplate& ft, const pb::Message& msg) {
  while (buf.size() < offset + ft.size) buf.push_back(0);
  std::string scratch;
  const std::string& s = msg.GetReflection()->GetStringReference(
      msg, ft.fd, &scratch);
...
void WriteFields(std::vector<uint8_t>& buf, size_t base,
                 const std::vector<FieldTemplate>& fields,
                 const pb::Message& msg);

void WriteCompositeField(std::vector<uint8_t>& buf, size_t base,
                         const FieldTemplate& ft, const pb::Message& msg) {
