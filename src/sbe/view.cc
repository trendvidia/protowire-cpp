...
GroupView View::Group(std::string_view name) const {
  ...
  for (const GroupTemplate& gt : *groups_) {
    ...
    if (gt.fd->name() == name) {
      auto group_data = data_.subspan(8 + pos + 4, static_cast<size_t>(entry_block) * count);
...
View View::Composite(std::string_view name) const {
  ...
  return View(data_, block_.subspan(ft->offset, ft->size), tmpl_, &ft->composite, nullptr);
}
