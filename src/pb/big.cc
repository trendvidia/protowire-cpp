...
bool ParseDigits(const std::string& s, size_t pos, size_t end,
...
void MarshalBigFloatMsg(const BigFloat& v, std::vector<uint8_t>& out) {
  ...
  if (v.exponent != 0) {
    wire::AppendTag(out, 2, wire::kVarint);
    wire::AppendVarint(
        out, wire::EncodeZigZag(static_cast<int64_t>(v.exponent)));
  }
  wire::AppendTag(out, 3, wire::kVarint);
...
Status UnmarshalBigIntMsg(std::span<const uint8_t> data, BigInt& out) {
  ...
  return ReadFields(
      data, out,
      [](std::span<const uint8_t> d, wire::FieldNumber num, wire::WireType,
         BigInt& v, int& consumed) -> Status {
        if (num == 1) {
...
Status UnmarshalBigFloatMsg(std::span<const uint8_t> data, BigFloat& out) {
  ...
  return ReadFields(
      data, out,
      [](std::span<const uint8_t> d, wire::FieldNumber num, wire::WireType,
         BigFloat& v, int& consumed) -> Status {
        if (num == 1) {
          std::span<const uint8_t> b;
          int n = wire::ConsumeBytes(d, b);
